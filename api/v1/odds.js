...
function americanFromDecimal(decimalOdds) {
  // Provider sometimes returns suspended/locked lines as decimal odds of 1
  // (or otherwise non-finite/invalid values). Treat those as "no price"
  // rather than let them produce -Infinity / NaN downstream.
  ...
  const books = (event.bookmakers || [])
    ...
    .map((bm) => {
      ...
      (h2h?.outcomes || []).forEach((o) => {
        const american = americanFromDecimal(o.price);
        if (american != null) prices[outcomeKeyFor(o.name)] = american;
      });
...
async function fetchFromProvider(sportKey, apiKey) {
  // Explicitly request DECIMAL odds — our americanFromDecimal() below expects
  // decimal input. (Previously this requested oddsFormat=american and then
  // ran that already-American number through the decimal conversion again,
  // producing nonsense values like +349900.)
  const url = `${PROVIDER_BASE}/sports/${sportKey}/odds?apiKey=${apiKey}&regions=us,eu&markets=h2h&oddsFormat=decimal`;
